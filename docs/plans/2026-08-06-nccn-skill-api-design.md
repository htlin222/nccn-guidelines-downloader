...
`sha256(bearer)` 查 `api_keys.hash`（UNIQUE 索引，`rows_read=1`），結果快取在
isolate 記憶體 10 秒。撤銷最壞延遲就是這 10 秒（實測 8 秒）。

**原本的設計在這裡是錯的，而且上線後才發現。** 原本多疊一層 KV（60 秒），撤銷時
主動刪掉那個 KV key，看起來滴水不漏。生產實測：D1 已標記撤銷、KV 的 key 也確實
不見了，被撤銷的金鑰仍暢行超過 24 秒 —— KV 的讀取在邊緣節點另有一層快取，而 KV
本來就是最終一致的，所以「這把金鑰死了」是照 KV 的時程傳播，不是照你的。

得到的規則：**快取層可以放資料，不能放「這個人能不能進來」。** 拿掉 KV 之後，
每個 isolate 每 10 秒多一次 `rows_read=1` 的查詢，對這種流量是零成本。
`test/apiflow.test.js` 有一條測試釘住「`apikey:` 開頭的東西永遠不寫進 KV」。

失敗的驗證也進記憶體快取（否則無效金鑰猛打等於直接壓 D1），代價是任何人都能往
那個 Map 裡塞東西，所以有 500 筆上限、滿了整個倒掉。
